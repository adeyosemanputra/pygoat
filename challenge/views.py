...
class DoItFast(View):
    ...
    def post(self, request, challenge):
        ...
        labels = {
            "traefik.enable": "true",
            f"traefik.http.routers.{lab_slug}.rule": f"Host(`{lab_slug}.lvh.me`)",
            f"traefik.http.middlewares.{lab_slug}-auth.forwardauth.address": "http://pygoat-web-1:8000/challenge/auth-check/",
            f"traefik.http.middlewares.{lab_slug}-auth.forwardauth.trustForwardHeader": "false",
...
def _sanitize_username(username: str) -> str:
    return "".join(ch for ch in username if ch.isalnum() or ch in "-")

def _sanitize_image_name(name: str) -> str:
    return "".join(ch for ch in name if ch.isalnum() or ch in "-")
    ...
    try:
        try:
            ...
            labels = {
                ...
                f"traefik.http.middlewares.{container_name}-auth.forwardauth.trustForwardHeader": "false",
            ...
            container = client.containers.run(
                ...
                detach=True,
                labels=labels,
                network=getattr(settings, "DOCKER_NETWORK", "pygoat_net"),
                mem_limit="512m",
