    ...
    if request.method == "POST":
        ...
        if action == "check_dependencies":
            ...
            return render_template(
                "lab.html", dependencies=dependencies, show_dependencies=True
            )
            ...
            if feature == "process_data":
                ...
                result = malicious_lib.collect_data(
                    {"action": "config_access", "config": config_data}
                )
...
def api_check_dependencies():
    ...
    return jsonify({"dependencies": dependencies, "total": len(dependencies)})
