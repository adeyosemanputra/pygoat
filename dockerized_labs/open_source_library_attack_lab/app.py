...
def get_version_details(version):
    ...
    version_data = next(
        (v for v in COMPROMISE_TIMELINE["versions"] if v["version"] == version), None
    )
    if version_data:
...
def compare_versions():
    ...
    version1 = next(
        (v for v in COMPROMISE_TIMELINE["versions"] if v["version"] == v1), None
    )
    version2 = next(
        (v for v in COMPROMISE_TIMELINE["versions"] if v["version"] == v2), None
    )
